...
const baseRoute = "master-data/semester";
...
const route = (prop) => {
  ...
  prop.app.post(
    ...
    async (req, res) => {
      try {
        ...
        // Check if semester already exists (by Khmer name)
        const existingSemester = await Model.findOne({
          name: name.trim(),
          deleted: false,
        });
        ...
        if (existingSemester) {
          return res.status(400).json({
            success: false,
            message: "ឆមាសនេះមានរួចហើយក្នុងប្រព័ន្ធ!",
          });
        }

        // Check if semester already exists (by English name)
        const existingSemesterEng = await Model.findOne({
          name_in_eng: name_in_eng.trim(),
          deleted: false,
        });

        if (existingSemesterEng) {
          return res.status(400).json({
            success: false,
            message: "ឆមាសនេះមានរួចហើយក្នុងប្រព័ន្ធ!",
          });
        ...
        // Log
        await logActivity({
          title: `ឆមាសថ្មី: ${name} (${name_in_eng}) ត្រូវបានបង្កើត!`,
          description: `បង្កើតដោយគណនី: ${user_data.firstname + " " + user_data.lastname}`,
          categoryTitle: "master_data_semester",
          createdBy: userId,
          req,
        });
        ...
        res.status(200).json({
          success: true,
          data: saveData,
          message: `ឆមាសថ្មី: ${name} (${name_in_eng}) ត្រូវបានបង្កើត!`,
        });
      } catch (error) {
        ...
        res.status(500).json({
          success: false,
          message: "មានបញ្ហាក្នុងការបង្កើតឆមាស! សូមព្យាយាមម្តងទៀត",
    ...
    async (req, res) => {
      try {
        ...
        if (name !== undefined && name !== null) {
          ...
          const duplicateSemester = await Model.findOne({
            name: name.trim(),
            deleted: false,
            _id: { $ne: id },
          });
        ...
        if (name_in_eng !== undefined && name_in_eng !== null) {
          ...
          const duplicateSemesterEng = await Model.findOne({
            name_in_eng: name_in_eng.trim(),
            deleted: false,
            _id: { $ne: id },
          });

          if (duplicateSemesterEng) {
            return res.status(400).json({
              success: false,
              message: "ឆមាសនេះមានរួចហើយក្នុងប្រព័ន្ធ!",
            });
        ...
        // Log
        await logActivity({
          title: `ឆមាស: ${updatedData.name} (${updatedData.name_in_eng}) ត្រូវបានកែប្រែ!`,
        ...
        res.status(500).json({
          success: false,
          message: "មានបញ្ហាក្នុងការកែប្រែឆមាស!",
          error: process.env.NODE_ENV === "development" ? error.message : undefined,
        });
      }
    },
  ...
  // ==========================================
  // SOFT DELETE - Soft delete semester (move to trash)
  // ==========================================
  prop.app.delete(
    `${urlAPI}/:id`,
    prop.api_auth,
    ...
    async (req, res) => {
      try {
        ...
        // Check if semester exists
        const semester = await Model.findOne({
          _id: id,
          deleted: false,
        });
        ...
        // Soft delete
        const updatedSemester = await Model.findByIdAndUpdate(
          id,
          {
            deleted: true,
            deleted_at: new Date(),
        ...
        // Log
        await logActivity({
          title: `${semester.name} (${semester.name_in_eng}) ត្រូវបានផ្លាស់ទៅធុងសំរាម!`,
  ...
  // ==========================================
  // RESTORE - Restore soft deleted semester
  // ==========================================
  prop.app.put(
    `${urlAPI}/restore/:id`,
    prop.api_auth,
    ...
    async (req, res) => {
      try {
        ...
        // Check if semester exists and is deleted
        const semester = await Model.findOne({
          _id: id,
          deleted: true,
        });
        ...
        // Restore
        const restoredSemester = await Model.findByIdAndUpdate(
          id,
          {
            deleted: false,
            deleted_at: null,
        ...
        // Log
        await logActivity({
          title: `${semester.name} (${semester.name_in_eng}) ត្រូវបានស្ដារមកវិញ!`,
  ...
  // ==========================================
  // DELETE FOREVER - Permanently delete semester
  // ==========================================
  prop.app.delete(
    `${urlAPI}/forever/:id`,
    prop.api_auth,
    ...
    async (req, res) => {
      try {
        ...
        // Store semester name for log before deletion
        const semesterName = semester.name;
        const semesterNameEng = semester.name_in_eng;
        ...
        // Log
        await logActivity({
          title: `${semesterName} (${semesterNameEng}) ត្រូវបានលុបចោលជាអចិន្ត្រៃយ៍!`,
  ...
  // ==========================================
  // GET TRASH - Get all soft deleted semesters
  // ==========================================
  prop.app.get(
    `${urlAPI}-trash`,
    prop.api_auth,
